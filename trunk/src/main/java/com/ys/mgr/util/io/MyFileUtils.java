...
//    }
    ...
    public static String path(String path) {
        String p = MyStringUtils.replace(path, "\\", "/");
        p = MyStringUtils.join(MyStringUtils.split(p, "/"), "/");
        if (!MyStringUtils.startsWithAny(p, "/") && MyStringUtils.startsWithAny(path, "\\", "/")) {
            p += "/";
        }
        if (!MyStringUtils.endsWithAny(p, "/") && MyStringUtils.endsWithAny(path, "\\", "/")) {
            p = p + "/";
        }
     ...
     * @return
     */
    public static String getDirByMonth(String pdir) {
        if (MyStringUtils.isBlank(pdir))
            return null;
        SimpleDateFormat formater = new SimpleDateFormat("yyyyMM");
        String path = MyStringUtils.replace(pdir, "\\", "/");
        if (!path.endsWith("/")) {
            path += "/";
        }
