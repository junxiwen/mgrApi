...
//	}
     ...
     * 私有构造函数
     */
    private IPSeekerOld(String ipDataFile) {
        ...
        b3 = new byte[3];
        try {
            ipFile = new RandomAccessFile(ipDataFile, "r");
        } catch (FileNotFoundException e) {
            System.out.println("IP地址信息文件" + ipDataFile + "没有找到，IP显示功能将无法使用");
            ipFile = null;
        }
        // 如果打开文件成功，读取文件头信息
        if (ipFile != null) {
    ...
    private static IPSeekerOld instance = null;

    /**
     * @param ipDataFile 实际上只能在第1次起作用
     *
     * @return 单一实例
     *
     */
    public static IPSeekerOld getInstance(String ipDataFile) {
        // 单一模式实例
        if (instance == null) {
            instance = new IPSeekerOld(ipDataFile);
        }
        return instance;
    }
     ...
     * @return IPLocation结构
     */
    private IPLocation findIPLocation(byte[] ip) {
    ...
    public IPLocation getIPLocatioin(byte[] ip) throws Exception {
        // 检查ip地址文件是否正常
        if (ipFile == null)
     ...
     * @throws Exception
     */
    public String getArea(byte[] ip) throws Exception {
     ...
     * @throws Exception
     */
    public String getArea(String ip) throws Exception {
...
//	}
     ...
     * @return
     */
    public String readStringFromMbb(int offset) {
        try {
            mbb.position(offset);
            int i;
    ...
    public String getAddress(String ip) throws Exception {
        IPLocation location = findIPLocation(Utils.getIpByteArrayFromString(ip));
